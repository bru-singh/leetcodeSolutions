...
public:
    int longestValidParentheses(string s) {
        ...
        // i.e any valid parenthsis has already been incountered, and we need to start after
        // the current index i.
        

        // initially we can start from 0,which is after -1
        // so we push -1 into the stack
        st.push_back(-1);
        for(int i=0;i<n;i++){
            if(s[i]=='('){
                st.push_back(i);
            ...
            if(st.size()==0){
                // start fresh after the index i;
                st.push_back(i);
            }
            else{
                // what is the length after the last start position
                // st.top()
